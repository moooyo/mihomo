...
type cache struct {
	...
	// epoch increments on every Flush. A resolver captures it BEFORE loading any
	// runtime snapshot and hands it back to Put; a mismatch means a flush landed
	// while the query was in flight, so an answer computed under the old rules
	// is returned to the client but not written into the new generation.
	epoch uint64
}
...
func newCache(max int) *cache {
	...
	return &cache{m: make(map[cacheKey]cacheEntry), max: max, now: time.Now}
}

// Epoch reports the current flush generation. Nil-safe.
...
func (c *cache) put(k cacheKey, msg *D.Msg, ttl time.Duration, epoch uint64, meta cacheMeta) {
	...
	c.m[k] = cacheEntry{msg: msg.Copy(), expiry: c.now().Add(ttl), meta: meta}
}
...
// exist -- which is the case immediately after a flush marks everything expired
// -- and a full scan under a flood would make eviction the bottleneck.
...
// Flush invalidates every entry after a rule, policy, or upstream swap.
//
// It expires entries in place rather than dropping the map, so get treats them
// as misses and the name re-resolves under the new generation while getStale
// can still serve them. Dropping the map outright would wipe the last known
// answers, and a reload that lands during a full upstream outage would then
// turn every name into SERVFAIL. Nil-safe.
func (c *cache) Flush() {
	if c == nil {
		return
	}
