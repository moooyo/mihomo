...
var (
	errQUICCaptureClosed = errors.New("5gpn/engine: the QUIC capture bridge is closed")
	...
	errQUICAssociationTaken = errors.New("5gpn/engine: the client address already has a captured association elsewhere")
)
...
func (b *quicBridge) WriteTo(p []byte, addr net.Addr) (int, error) {
	...
	if !ok {
		return 0, fmt.Errorf("5gpn/engine: QUIC capture cannot address %v", addr)
	}
	b.mu.RLock()
...
// process, because the engine is assembled once: fivegpn.StartInterception runs at
...
func (p *interceptProxy) captureQUIC(metadata *C.Metadata) (C.PacketConn, error) {
	...
	if origin == nil {
		return nil, fmt.Errorf("5gpn/engine: captured association for %s has no destination address", metadata.Host)
	}
	client := metadata.SourceAddrPort()
	if !client.IsValid() {
		return nil, fmt.Errorf("5gpn/engine: captured association for %s has no client address", metadata.Host)
	}
	...
	log.Debugln("[5GPN] captured QUIC association for %s from %s", metadata.Host, client)
...
func (p *interceptProxy) quicListener() (*quicBridge, error) {
	...
	if err != nil {
		_ = bridge.Close()
		return nil, fmt.Errorf("5gpn/engine: QUIC capture listener: %w", err)
	}
	...
	go func() {
		if err := server.ServeListener(listener); err != nil && !errors.Is(err, http.ErrServerClosed) {
			log.Warnln("[5GPN] QUIC capture listener stopped: %v", err)
		}
	}()
	p.quicBridge = bridge
	p.quicServer = server
	log.Infoln("[5GPN] QUIC capture listener started")
	return bridge, nil
}
