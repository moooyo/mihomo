...
func validateModules(modules []Module) error {
	...
	for index, module := range modules {
		...
		for _, rule := range module.Scripts {
			...
			// what the interception config validator runs, so nothing caught them.
...
// live resolver groups inside the 5gpn DNS subsystem, dialled verbatim on every
// resolution of the mapped name, so a private or link-local address here is
// the same capability the address form is refused for.
