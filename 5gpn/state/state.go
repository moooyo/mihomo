...
func New[T any](path string, zero T) (*Doc[T], error) {
	...
	case err == nil:
		...
		if err := DecodeJSONBytes(raw, MaxDocumentBytes, &value); err != nil {
			...
			return nil, fmt.Errorf("5gpn/state: %s is unreadable: %w", path, err)
		}
...
func clonePrivate[T any](value T) (T, error) {
	...
	if err != nil {
		return zero, fmt.Errorf("5gpn/state: clone document: %w", err)
	}
	var out T
...
func (d *Doc[T]) write(value T) error {
	...
	if err != nil {
		return fmt.Errorf("5gpn/state: marshal %s: %w", d.path, err)
	}
...
func writeFileMode(path string, data []byte, mode os.FileMode, syncDirectory func(string) error) error {
	...
	if err != nil {
		return fmt.Errorf("5gpn/state: create temp in %s: %w", dir, err)
	}
	tmpName := tmp.Name()
...
func syncDir(dir string) error {
	...
	f, err := os.Open(dir)
	if err != nil {
		return fmt.Errorf("5gpn/state: open %s: %w", dir, err)
	}
	defer f.Close()
	if err := f.Sync(); err != nil {
		return fmt.Errorf("5gpn/state: fsync %s: %w", dir, err)
	}
	return nil
}
...
func Dir(home string) (string, error) {
	dir := filepath.Join(home, "5gpn")
	if err := os.MkdirAll(dir, 0o711); err != nil {
		return "", fmt.Errorf("5gpn/state: create %s: %w", dir, err)
	}
	// MkdirAll leaves an existing directory's mode alone, so enforce the
	// traversal contract even when the installer created it first.
	if err := os.Chmod(dir, 0o711); err != nil {
		return "", fmt.Errorf("5gpn/state: chmod %s: %w", dir, err)
	}
	return dir, nil
}
