...
func (s *Service) runWithClient(ctx context.Context, doc Document, c *client) {
	...
	if err != nil {
		...
		log.Warnln("[GPN/BOT] cannot reach Telegram: %v", redactToken(err.Error(), doc.Token))
	} else {
		s.setState("running", "")
		log.Infoln("[GPN/BOT] connected as @%s", me.Username)
	}
...
func (s *Service) reply(ctx context.Context, c *client, chatID int64, text string) {
	...
	if err := c.sendMessage(sendCtx, chatID, text); err != nil && ctx.Err() == nil {
		log.Warnln("[GPN/BOT] reply failed: %v", redactToken(err.Error(), c.token))
	}
}
...
var errAlertUndeliverable = errors.New("gpn/bot: no admin could be notified")
...
func (s *Service) notifyAdmins(ctx context.Context, c *client, admins []int64, text string) error {
	...
	for _, id := range admins {
		...
		log.Warnln("[GPN/BOT] alert to %s failed: %v", strconv.FormatInt(id, 10), redactToken(err.Error(), c.token))
	}
	if delivered == 0 {
