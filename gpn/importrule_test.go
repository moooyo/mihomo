...
// The fork's real cost is not the size of gpn/ -- it is how many upstream-owned
...
// from gpn/policy or a document from gpn/state, reaches for it directly from an
// upstream package, and the façade quietly stops being one. This test is the
// thing that notices.
//
// Upstream packages may import "gpn". They may not import anything beneath it.
func TestUpstreamPackagesImportOnlyTheFacade(t *testing.T) {
	const module = "github.com/metacubex/mihomo"
	const facade = module + "/gpn"
	...
	for dec.More() {
		...
		// gpn's own packages may import each other freely.
	...
	if len(offenders) > 0 {
		t.Errorf("upstream packages reached past the gpn façade:\n  %s\n\n"+
			"Export what is needed from package gpn instead. Every direct import of a\n"+
			"gpn sub-package is an upstream-owned file a future rebase has to reconcile.",
