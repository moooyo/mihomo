...
var ErrNoTrafficPolicy = errors.New("gpn/dial: traffic policy not initialised")
...
func transformedMetadata(network C.NetWork, host string, port int) (*C.Metadata, error) {
	if port < 1 || port > 65535 {
		return nil, fmt.Errorf("gpn/dial: invalid destination port %d", port)
	}
	metadata := &C.Metadata{
		NetWork: network,
		Type:    C.INNER,
	...
	if err := metadata.SetRemoteAddress(net.JoinHostPort(host, strconv.Itoa(port))); err != nil {
		return nil, fmt.Errorf("gpn/dial: invalid transformed target: %w", err)
	}
	return metadata, nil
}
...
func authorizeEgress(state *trafficAuthorization, metadata *C.Metadata, owner string, ownerOnly bool) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("gpn/dial: transformed egress denied: %w", err)
	}
	if proxy == "" {
		return "", nil
	}
	if state.proxyExists == nil || !state.proxyExists(proxy) {
		return "", fmt.Errorf("gpn/dial: transformed egress proxy %q is unavailable", proxy)
	}
	return proxy, nil
}
