...
func parseECS(raw string) (netip.Prefix, error) {
	...
	if strings.Contains(raw, "/") {
		...
		if err != nil {
			return netip.Prefix{}, fmt.Errorf("gpn/dns: ecs %q is not a valid CIDR: %w", raw, err)
		}
		return p.Masked(), nil
	}
	addr, err := netip.ParseAddr(raw)
	if err != nil {
		return netip.Prefix{}, fmt.Errorf("gpn/dns: ecs %q is not an address or CIDR: %w", raw, err)
	}
	addr = addr.Unmap()
	bits := 56
	if addr.Is4() {
