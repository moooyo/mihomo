...
func EnsureDocument(path string) error {
	...
	case !errors.Is(err, os.ErrNotExist):
		return fmt.Errorf("gpn/engine: stat %s: %w", path, err)
	}
	raw, err := json.MarshalIndent(DefaultDocument(), "", "  ")
	if err != nil {
		return fmt.Errorf("gpn/engine: marshal the default document: %w", err)
	}
	...
	if _, err := decodeConfig(raw); err != nil {
		return fmt.Errorf("gpn/engine: the default document does not validate: %w", err)
	}
