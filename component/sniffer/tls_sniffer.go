	...
	for offset < extensionsEnd {
		...
		if extension == 0x00 { /* extensionServerName */
			...
			for offset < namesEnd {
				...
				if nameType == 0 {
					// Domain rules consume lower-case names. DNS names are
					// case-insensitive, so preserving mixed-case SNI here would
					// let the same host bypass DOMAIN matching.
					serverName := strings.ToLower(string(data[offset:nameEnd]))
